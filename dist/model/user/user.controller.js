...
const getTicketHandler = async (_, res) => {
    const tickets = await (0, user_repository_1.findAllTickets)();
...
const getTicketByIdHandler = async (req, res) => {
    const { id } = req.params;
    const ticket = await (0, user_service_1.findTicketByIdService)(id);
...
const getTicketByMachineIdHandler = async (req, res) => {
    ...
    const tickets = await (0, user_service_1.findTicketsByMachineIdService)(machineId);
    return (0, response_1.successRes)({ res, message: 'success', data: { tickets } });
};
...
const createTicketHandler = async (req, res) => {
    ...
    const ticket = await (0, user_service_1.createTicketService)({ data: { productId, problem, priority, status, problemDetail, isPublished } });
...
const updateTicketHandler = async (req, res) => {
    const { id } = req.params;
    const { productId, problem, priority, status, problemDetail, isPublished } = req.body;
    const ticket = await (0, user_service_1.updateTicketService)({ ticketId: id, data: { productId, problem, priority, status, problemDetail, isPublished } });
...
const deleteTicketHandler = async (req, res) => {
    const { id } = req.params;
    const ticket = await (0, user_service_1.deleteTicketService)({ ticketId: id });
