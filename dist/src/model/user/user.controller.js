...
const verifyUserHandler = async (req, res) => {
    const { id } = req.params;
    const user = await (0, user_repository_1.verifyUser)(id);
...
const unverifyUserHandler = async (req, res) => {
    const { id } = req.params;
    const user = await (0, user_repository_1.unverifyUser)(id);
...
const deleteUserHandler = async (req, res) => {
    const { id } = req.params;
    const user = await (0, user_repository_1.deleteUserById)(id);
